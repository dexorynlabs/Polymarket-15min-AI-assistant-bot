...
function parsePriceToBeatText(market) {
...
export function extractStrikePrice(market) {
  const directKeys = [
    "priceToBeat",
    "price_to_beat",
    "strikePrice",
  ...
  while (stack.length) {
    ...
    for (const [key, value] of entries) {
      ...
      if (!/(price|strike|threshold|target|beat)/i.test(keyName)) continue;

      const parsed = toNumber(value);
      if (parsed !== null && parsed > 1000 && parsed < 2_000_000) return parsed;
    }
  }

  return parsePriceToBeatText(market);
}
